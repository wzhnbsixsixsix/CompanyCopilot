Title: RAG - AgentScope

URL Source: https://doc.agentscope.io/zh_CN/tutorial/task_rag.html

Published Time: Tue, 03 Mar 2026 02:40:26 GMT
...
Hide table of contents sidebar
 ...
 [Skip to content](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#furo-main-content)
...
[Back to top](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#)

[View this page](https://doc.agentscope.io/zh_CN/_sources/tutorial/task_rag.rst.txt "View this page")
...
[Go to the end](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#sphx-glr-download-tutorial-task-rag-py) to download the full example code.

RAG[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#rag "Link to this heading")
=========================================================================================
...
RAG 集成方式比较[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id11 "Link to this table")| 集成方式 | 描述 | 优点 | 缺点 |
| --- | --- | --- | --- |
| 智能体自主控制 | 以工具调用方式让智能体自主决定何时进行查询，查询什么关键字 | * 与 ReAct 算法契合，灵活性高 * 智能体能够根据当前的上下文改写查询关键词 * 避免在不必要时发生查询 | 对 LLM 模型能力要求较高 |
| 通用方式 | 每次在 `reply` 函数开始时固定进行查询，并将检索结果整合到提示（prompt）中 | * 实现简单 * 对 LLM 模型能力要求低 | * 每次都会运行查询，因此会引入过多不必要的查询检索 * 查询数据库较大时，回复延迟较高 |

备注

作为开源框架，AgentScope 的目标是让开发过程更简单也更有趣。因此，AgentScope 的设计中并不强制要求使用内置的 RAG 实现，同时支持、鼓励开发者集成现有的 RAG 实现或第三方 RAG 框架。
...
使用 RAG 模块[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id2 "Link to this heading")
-----------------------------------------------------------------------------------------------
...
我们将持续在 AgentScope 中集成新的向量数据库和数据读取模块。详情请查看我们的 [开发路线图](https://github.com/orgs/agentscope-ai/projects/2)，同时也欢迎贡献代码！
...
由于并不存在一个 “one-for-all” 的数据读取和分块方法，特别像是 PDF 和 Word 这类复杂格式的文档。 因此，AgentScope 鼓励开发者根据自己的数据格式实现自定义的 reader。 只需要继承 `BaseReader` 类，并实现 `__call__` 方法即可。

在数据分块后，接下来需要将数据块添加到知识库中。 在 AgentScope 中，知识库的初始化需要提供 **嵌入模型** 和 **向量存储** （即向量数据库） 的对象。 AgentScope 目前内置支持基于 [Qdrant](https://qdrant.tech/) 实现的向量存储，以及一个知识库的基础实现 `SimpleKnowledge`。 具体使用方式如下：
...
检索到的 Document 对象：
Document(metadata=DocMetadata(content={'type': 'text', 'text': '我父亲的名字是李强，是一名医生，我的母亲是陈芳芳，是一名教师，她总是指导我学习。'}, doc_id='547e45879e96438c19ddb0f06bcf750ead3cc20e079458880f3533bcb2300a13', chunk_id=2, total_chunks=5), id='hMbvCtYiQwH6T7GQPjr2EB', embedding=None, score=0.5971063116560947)

Document(metadata=DocMetadata(content={'type': 'text', 'text': '我的名字是李明，今年28岁。'}, doc_id='547e45879e96438c19ddb0f06bcf750ead3cc20e079458880f3533bcb2300a13', chunk_id=0, total_chunks=5), id='4jPq5CGZR7uXKgHoKPbXYp', embedding=None, score=0.5449764333507582)
...
自定义 RAG 组件[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id6 "Link to this heading")
------------------------------------------------------------------------------------------------

AgentScope 支持并鼓励开发者自定义 RAG 组件，包括 reader、知识库和向量数据库。 具体来说，我们提供了以下基类用于自定义：

RAG 基类[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id12 "Link to this table")| 基类 | 描述 | 抽象方法 |
| --- | --- | --- |
| `ReaderBase` | 所有 reader 的基类 | `__call__` |
| `VDBStoreBase` | 向量数据库的基类 | `add` `search` `get_client` (可选) `delete` (可选) |
| `KnowledgeBase` | 知识库的基类 | `retrieve` `add_documents` |

`VDBStoreBase` 中的 `get_client` 方法允许开发者访问底层向量数据库的完整功能。 这样，他们就可以基于向量数据库实现更高级的功能，例如建立索引、高级搜索等。

与 ReActAgent 集成[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#reactagent "Link to this heading")
------------------------------------------------------------------------------------------------------------

接下来，我们将演示如何以智能体自主控制（agentic）和通用（generic）两种方式将 RAG 模块与 AgentScope 中的 `ReActAgent` 集成。

### 智能体自主控制[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id7 "Link to this heading")

在智能体自主控制的方式中，ReAct 智能体可以自主决定何时检索知识以及检索的查询内容。 将 RAG 模块与 AgentScope 中的 `ReActAgent` 集成非常简单，只需将知识库的 `retrieve_knowledge` 方法注册为工具，并为该工具提供适当的描述即可。
...
第一次回复:
Friday: 那真是太好了！有好朋友是一件非常幸福的事情。你们经常一起做什么呢？

第二次回复:
Friday: {
    "type": "tool_use",
    "id": "call_70b70112410d42feb15451",
    ...
    "raw_input": "{\"query\": \"李明的父亲\", \"limit\": 5, \"score_threshold\": null}"
}
system: {
    "type": "tool_result",
    "id": "call_70b70112410d42feb15451",
...
Friday: 根据我找到的信息，李明的父亲名叫李强，他是一名医生。如果你需要更多关于李明或他父亲的详细信息，请告诉我。

在上面的例子中，我们模拟了正常与智能体交流过程。第一次的交流我们提供了“李明”的名字作为上下文内容。 第二次提问时，我们的问题是“你知道他父亲是谁吗？”， 我们希望智能体能够利用上下文历史信息改写查询，使其更具体，更好的进行检索，例如改写为“李明的父亲是谁？”或“李明的父亲”。

更进一步，结合 [计划](https://doc.agentscope.io/zh_CN/tutorial/task_plan.html#plan) 模块，我们可以让智能体实现更加复杂的查询改写和多轮检索。

### 通用方式[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id8 "Link to this heading")

`ReActAgent` 还以一种更加通用的方式集成了 RAG 模块， 它在每次 `reply` 函数开始执行时检索知识，并将检索到的知识附加到用户消息的提示中。
...
Friday: 根据提供的信息，李明的父亲是李强，他是一名医生。
...
多模态 RAG[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id9 "Link to this heading")
---------------------------------------------------------------------------------------------
...
然后我们可以构建一个多模态知识库，构建过程与纯文本知识库类似。只是将 reader 和嵌入模型替换为多模态版本即可。 在下面的例子中，我们使用了 `ImageReader` 和 `DashScopeMultiModalEmbedding`。 同时，这里我们使用多模态模型 `qwen3-vl-plus` 作为智能体的语言模型。
...
进一步阅读[¶](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id10 "Link to this heading")
--------------------------------------------------------------------------------------------

*   [嵌入(Embedding)](https://doc.agentscope.io/zh_CN/tutorial/task_embedding.html#embedding)

*   [计划](https://doc.agentscope.io/zh_CN/tutorial/task_plan.html#plan)

**Total running time of the script:** (0 minutes 31.328 seconds)

[`Download Jupyter notebook: task_rag.ipynb`](https://doc.agentscope.io/zh_CN/_downloads/dc35bacf6284d8bd4018ad68eeecf856/task_rag.ipynb)

[`Download Python source code: task_rag.py`](https://doc.agentscope.io/zh_CN/_downloads/680c828412e9013a9af698222658a54e/task_rag.py)

[`Download zipped: task_rag.zip`](https://doc.agentscope.io/zh_CN/_downloads/c84da3ed7c258615a72afe66c8852944/task_rag.zip)

[Gallery generated by Sphinx-Gallery](https://sphinx-gallery.github.io/)

[Next AgentScope Studio](https://doc.agentscope.io/zh_CN/tutorial/task_studio.html)[Previous 计划](https://doc.agentscope.io/zh_CN/tutorial/task_plan.html)
...
*   [RAG](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#)
    *   [使用 RAG 模块](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id2)
    *   [自定义 RAG 组件](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id6)
    *   [与 ReActAgent 集成](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#reactagent)
        *   [智能体自主控制](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id7)
        *   [通用方式](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id8)

    *   [多模态 RAG](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id9)
    *   [进一步阅读](https://doc.agentscope.io/zh_CN/tutorial/task_rag.html#id10)